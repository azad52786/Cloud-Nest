			...
			<Sheet open={open} onOpenChange={setOpen}>
				...
				<SheetContent className=" flex flex-col gap-2  bg-slate-800">
					<SheetTitle className=" mt-5">
						<div className=" text-slate-100 break-words bg-sky-500 my-3 flex items-center gap-2 rounded-full p-2 sm:justify-center lg:justify-start ">
							<Image
							...
							<div className="sm:hidden lg:block">
								...
								<p className=" text-sm font-normal">{userDetails?.email}</p>
							</div>
						</div>
							...
							{navItems.map(({ name, url, icon }) => {
								return (
									<div key={name}>
										<li
											className={cn(
												"flex gap-3  text-white  lg:w-full items-center grow-[1] justify-start px-[30px] h-[54px] rounded-full ",
												pathname === url &&
										...
										>
											...
											/>
											<p className="block text-[16px] lg:hidden">{name}</p>
										</li>
									</div>
								);
							})}
